...
class TaskController extends Controller
{
    ...
    public function store(CreateTaskRequest $request)
    {
        ...
        $taskStartDate = date("Y-m-d", strtotime($request['task_start_date']));
        $taskEndDate = date("Y-m-d", strtotime($request['task_end_date']));
        $plannedStartDate = date("Y-m-d", strtotime($request['planned_start_date']));
        $plannedEndDate = date("Y-m-d", strtotime($request['planned_end_date']));
    ...
    public function getTaskForTaskBoard(Request $request)
    {
        // --
        // Check role/permission
        if (!AdminHelper::can_action(45, 'view')) {
            return response()->json("Access denied", 403);
        }
    ...
    public function updateStatusList(Request $request)
    {
        // --
        // Check role/permission
        if (!AdminHelper::can_action(45, 'edit')) {
            return response()->json("Access denied", 403);
        }

        $task = $this->taskRepo->updateStatusList($request);
    ...
    {
        return $this->taskRepo->getTaskLastId();
    }
    ...
    {
        return $this->taskRepo->getSubTaskCountByParent($parent_task_id);
    }
     ...
     * @return Response
     */
    public function getTaskForDashboard(Request $request)
    {
        // --
        // Check role/permission
        if (!AdminHelper::can_action(44, 'view')) {
        ...
        return $this->taskRepo->getTaskForDashboard($request);
    }

    /**
     * Get tasks count by status for dashboard chart.
     *
     * @param Request $request [Request for get tasks count]
     *
     * @return Response
     */
    public function getTaskCountByStatus(Request $request)
    {
        // --
        // Check role/permission
        if (!AdminHelper::can_action(44, 'view')) {
        ...
        return $this->taskRepo->getTaskCountByStatus($request);
    }
